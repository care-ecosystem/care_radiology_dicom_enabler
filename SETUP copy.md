...
- Visual Studio installed (built on Visual Studio 2019)
...
### Step 1: Setting up the CARE ecosystem

1. Clone and setup `care` repository from https://github.com/ohcnetwork/care
2. Clone and setup `care_fe` repository from https://github.com/ohcnetwork/care_fe
3. Clone and setup `care_radiology` repository inside `care`
4. Clone and setup `care_radiology_fe` repository

### Step 2: Clone the CARE Radiology DICOM Enabler

- For ready executable: https://github.com/care-ecosystem/care_radiology_dicom_enabler/tree/pranav
- For code: https://github.com/care-ecosystem/care_radiology_dicom_enabler/tree/main

### Step 3: Install additional tools

- Install MySQL and MySQL Workbench
- Download Visual Studio (not VS Code) from https://visualstudio.microsoft.com/downloads/
- Download and install DVTK Modality Emulator from https://dvtk-modality-emulator.software.informer.com/download/
...
2. Click on the Local Instance under MySQL Connections
3. Authenticate with your password
4. Run the following:

```sql
CREATE DATABASE plexus_mi2;
USE plexus_mi2;
```
...
In MySQL Workbench, open the Schemas tab in the left panel. Expand the database, right-click "Stored Procedures" and select "Create Stored Procedure". Add the following one by one:
...
proc_label: BEGIN
 ...
 Where sop_iuid = imageinstanceid;
    IF(getimageid > 0) THEN
        ...
        SET outreturnstatus = -1;
    ELSE
        Select  pk INTO getstudyid From study Where study_iuid = studyinstanceid;
        Select  num_series INTO getstudyseriescount From study Where study_iuid = studyinstanceid;
        Select  num_instances INTO getstudyimagecount From study Where study_iuid = studyinstanceid;

        IF(getstudyid > 0) THEN
            Select  pk INTO getseriesid From series Where series_iuid = seriesinstanceid;
            Select  num_instances INTO getseriesimagecount From series Where series_iuid = seriesinstanceid;
            IF(getseriesid > 0) THEN
                SET getstudyimagecount = getstudyimagecount + 1;
                SET getseriesimagecount = getseriesimagecount + 1;
                UPDATE STUDY SET updated_time = now(),num_instances=getstudyimagecount,study_status=studystatus WHERE study_iuid = studyinstanceid;
                UPDATE SERIES SET updated_time = now(),num_instances=getseriesimagecount WHERE series_iuid = seriesinstanceid;
                INSERT INTO INSTANCE (series_fk,sop_iuid,created_time,updated_time,sop_cuid) VALUES (getseriesid,imageinstanceid,now(),now(),sopclassuid);
            ELSE
                SET getstudyseriescount = getstudyseriescount + 1;
                SET getstudyimagecount = getstudyimagecount + 1;
                UPDATE STUDY SET updated_time = now(),num_series=getstudyseriescount,num_instances=getstudyimagecount,study_status=studystatus WHERE study_iuid = studyinstanceid;
                ...
                VALUES (getstudyid,seriesinstanceid,seriesno,modality,bodypart,series_desc,institution,stationname,department,1,now(),now());
                SET getseriesid = LAST_INSERT_ID();
                INSERT INTO INSTANCE (series_fk,sop_iuid,created_time,updated_time,sop_cuid) VALUES (getseriesid,imageinstanceid,now(),now(),sopclassuid);
            END IF;
        ELSE
            Select  pk INTO getpatientid From patient Where pat_id = patient_id;
            IF(getpatientid > 0) THEN
                UPDATE STUDY SET study_iuid = studyinstanceid , num_series = 1, num_instances = 1 , study_status=studystatus, updated_time = now() WHERE patient_fk = getpatientid;
                Select  pk INTO getstudyid From study Where study_iuid = studyinstanceid;
                INSERT INTO SERIES (study_fk,series_iuid,series_no,modality,body_part,series_desc,institution,station_name,department,num_instances,created_time,updated_time)
                VALUES (getstudyid,seriesinstanceid,seriesno,modality,bodypart,series_desc,institution,stationname,department,1,now(),now());
                SET getseriesid = LAST_INSERT_ID();
...
> After running this, the MySQL password for `root@localhost` will be `inzin@123`.

---

## Setting Up DICOM Enabler

1. Open the cloned repository in Windows File Explorer
2. Locate `CARE_DICOM_Enabler.exe`, right-click and **Run as Administrator**
3. Log into the application using the UI
4. Click **Install Services** followed by **Start Services**
5. Go to the **3rd tab** (SCP Settings) — change PAC server config if needed
6. If anything was updated, click **Save**, then go to the 1st tab and restart services (Stop → Start)
7. Go to the **4th tab** (Server List) — add Modality Emulator details:
   - Open the running Modality Emulator and go to "Configure Emulator"
   - System Name in Modality Emulator → Server Name in DICOM Enabler
   - AE Title → AE Title
   - Host Address: `127.0.0.1`
   - Port Number: `104`
8. Click **Add Server**
9. Check the **6th tab** (View Logs) — you should see worklist API calls at port 9000

> **Changing the backend URL:** Open `CARE_MWL_Service.exe.config` and update `careBaseUrl` inside `<appSettings>`.
>
> **403 Forbidden error:** Check `careToken` in the same config file. The default password defined in `plug_config.py` on the backend is `RANDOMSECRET`.

---

## Verifying It Works

1. Open `CARE_DICOM_Enabler.sln` in Visual Studio
2. In the project dropdown at the top, switch from `CARE_DICOM_Enabler` to `SampleStoreSCP`
3. Press `Fn + F5` to run the application
4. In the left path field, enter: `C:\Users\Admin\Downloads\FDE (2)\FDE\SCP`
5. In the right path field, enter the folder path containing your DICOM files
   - The accession number must be set to the last two groups of a UUID without hyphens
6. Click **Start** followed by **Push**
7. Check **Tab 6 (View Logs)** in the DICOM Enabler — worklist API calls on port 9000 confirm the full chain is working
...
These are the ports opened by the DICOM Enabler services after they are installed and started.

| Port | Service | Protocol | Direction | Purpose |
|---|---|---|---|---|
| **2008** | MWL SCP (`CARE_MWL_Service`) | DICOM | Inbound | Receives C-FIND requests from modalities for patient worklist |
| **2007** | Store SCP (`CARE_StoreSCP_Service`) | DICOM | Inbound | Receives DICOM images (C-STORE) from modalities |
| **configured** | Store SCU (`CARE_SCU_Service`) | DICOM | Outbound | Pushes stored images to an upstream DICOM node (no local port opened) |
| **104** | Modality Emulator (DVTK) | DICOM | Inbound | Standard DICOM port used by the emulator — not part of the Enabler itself |
| **9000** | CARE Backend API | HTTP | Outbound | MWL service calls this to fetch the patient worklist |

> Port values for MWL and Store SCP are read from `cfg/common.cfg` in the install directory (`mwlport`, `sscpport`). They can be changed from Tab 3 (SCP Settings) in the UI.

### How to verify ports are open

After starting services, run in a Windows Command Prompt:

```bat
netstat -ano | findstr "2007"
netstat -ano | findstr "2008"
```

You should see lines with `LISTENING` against both ports. If not, the service failed to start — check Tab 6 (Logs) in the Enabler.

### Testing MWL SCP (port 2008)

**Using DVTK Modality Emulator (recommended):**
1. Open DVTK Modality Emulator
2. Go to Configure Emulator
3. Set the SCP AE Title to `MODALITYSCP`, Host `127.0.0.1`, Port `2008`
4. Trigger a worklist query — the emulator should return the patient list from CARE

**Using DCMTK (command line):**
```bat
findscu -W -k "ScheduledProcedureStepSequence" 127.0.0.1 2008 -aec MODALITYSCP
```

### Testing Store SCP (port 2007)

**Using SampleStoreSCP (built into this repo):**
1. Open the solution in Visual Studio
2. Switch the startup project to `SampleStoreSCP`
3. Press `Fn + F5`
4. Set left path to the SCP folder, right path to a folder with DICOM files
5. Click **Start** then **Push**
6. Received files land in the `SCP` subfolder next to the service exe

**Using DCMTK (command line):**
```bat
storescu -aec STORAGESCP 127.0.0.1 2007 path\to\file.dcm
```

### Testing the CARE backend connection (port 9000)

Open a browser or use curl to hit the worklist endpoint directly:
```bat
curl -H "Authorization: Bearer YOUR_TOKEN" https://your-care-host:9000/api/v1/...
```

If this returns data, the MWL service will be able to serve worklist queries. If you get 403, check `careToken` in `CARE_MWL_Service.exe.config`.
...
| Issue | Fix |
|---|---|
| Services won't install | Run `CARE_DICOM_Enabler.exe` as **Administrator** |
| 403 Forbidden in logs | Check `careToken` in `CARE_MWL_Service.exe.config` — default is `RANDOMSECRET` |
| Wrong backend URL | Update `careBaseUrl` in `CARE_MWL_Service.exe.config` |
| DB connection fails | Ensure password is `inzin@123` after running the `ALTER USER` command |
| Database not visible in Workbench | Right-click Schemas panel → Refresh All |
